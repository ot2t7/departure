...
fn consume_string(
    ...
    let converted = OsString::from_vec(res.as_bytes().to_vec());
    return Ok(converted);
}
...
/// Lua 5.1 bytecode.
pub fn to_bytecode(src: &String, state: &Lua) -> Result<Cursor<Vec<u8>>, Error> {
...
pub fn deserialize<'a>(src: &String) -> Result<Function, ParserError> {
    let state = Lua::new();
